...
function bounded(boundingBox: BBox, options: BoundedStaticMapOptions = {}): string {
	...
	const endpoint = new URL(
		`maps/${encodeURIComponent(style)}/static/${boundingBox[0]},${boundingBox[1]},${
			boundingBox[2]
		},${boundingBox[3]}/${width}x${height}${scale}.${format}`,
		defaults.maptilerApiURL,
	)
...
function automatic(options: AutomaticStaticMapOptions = {}): string {
	...
	const endpoint = new URL(
		`maps/${encodeURIComponent(style)}/static/auto/${width}x${height}${scale}.${format}`,
		defaults.maptilerApiURL,
	)
