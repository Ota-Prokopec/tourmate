import { protectedProcedure } from '$src/middlewares/isAuthed'
...
export const get = protectedProcedure.query(async ({ ctx }): Promise<TGetAccountOutputData> => {
	...
	const [appwriteAccount, quiziAccount] = await Promise.all([account.get(), getUserInfoByUserId(ctx.user.$id)])
