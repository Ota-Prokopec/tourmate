...
export const getExperiencesByUserByMyId = protectedProcedure
	...
	.query(async ({ ctx, input }) => {
		...
		const userId = (await collections.userInfo.getDocument([Query.equal('myId', input.myId)])).userId
		const experiences = await collections.experience.listDocuments([Query.equal('userId', userId), Query.limit(100)])
